...
class BitcoinPredictorApp(QMainWindow):
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        """
        Daha gerçekçi tahmin fonksiyonu - döngüsel hareketler ve derin düzeltmelerle
        """
        try:
            ...
            market_cycles = {
                'bull': {
                    ...
                    'total_growth': (1.5, 3.0),  # %150-300 büyüme
                    'daily_volatility': (0.02, 0.05)
                },
                'bear': {
                    'duration': (60, 120),  # 2-4 ay
                    'total_decline': (0.2, 0.4),  # %60-80 düşüş
                    'daily_volatility': (0.03, 0.06)
                },
                'accumulation': {
                    'duration': (30, 90),  # 1-3 ay
                    'price_range': (0.1, 0.2),  # %10-20 aralık
                    'daily_volatility': (0.01, 0.03)
                },
                'distribution': {
                    'duration': (30, 60),  # 1-2 ay
                    'price_range': (0.05, 0.15),  # %5-15 aralık
                    'daily_volatility': (0.02, 0.04)
                }
            }
            ...
            cycle_day = 0
            cycle_duration = np.random.randint(*market_cycles[current_trend]['duration'])
            cycle_target = current_price
            
            # Son sequence'i al
            last_sequence = self.X_scaled[-60:].copy()
            
            # Halving cycle etkisi (4 yıllık)
            halving_cycle = 4 * 365
            halving_phase = 2 * np.pi * np.arange(days_to_predict) / halving_cycle
            
            for i in range(days_to_predict):
                try:
                    current_date = last_real_date + timedelta(days=i+1)
                    
                    # Döngü değişimi kontrolü
                    if cycle_day >= cycle_duration:
                        cycle_day = 0
                        
                        # Trend geçişleri
                        if current_trend == 'accumulation':
                            current_trend = 'bull'
                            cycle_target = current_price * np.random.uniform(*market_cycles['bull']['total_growth'])
                        elif current_trend == 'bull':
                            current_trend = 'distribution'
                            cycle_target = current_price * (1 + np.random.uniform(*market_cycles['distribution']['price_range']))
                        elif current_trend == 'distribution':
                            current_trend = 'bear'
                            cycle_target = current_price * (1 - np.random.uniform(*market_cycles['bear']['total_decline']))
                        else:  # bear
                            current_trend = 'accumulation'
                            cycle_target = current_price * (1 + np.random.uniform(*market_cycles['accumulation']['price_range']))
                        
                        cycle_duration = np.random.randint(*market_cycles[current_trend]['duration'])
                    
                    # Günlük volatilite
                    daily_volatility = np.random.uniform(*market_cycles[current_trend]['daily_volatility'])
                    
                    # Halving cycle etkisi
                    halving_effect = np.sin(halving_phase[i]) * 0.001
                    
                    # Fiyat değişimi hesaplama
                    if current_trend in ['bull', 'bear']:
                        # Trend yönünde hareket
                        direction = 1 if current_trend == 'bull' else -1
                        target_return = (cycle_target / current_price) ** (1 / (cycle_duration - cycle_day))
                        expected_change = (target_return - 1) * direction
                    else:
                        # Yatay hareket
                        range_width = market_cycles[current_trend]['price_range'][1]
                        expected_change = np.random.uniform(-range_width/2, range_width/2)
                    
                    # Rastgele değişim ekle
                    price_change = expected_change + np.random.normal(0, daily_volatility) + halving_effect
                    ...
                    current_price = np.clip(
                        current_price,
                        historical_min * 0.5,  # Minimum değer
                        historical_max * 5.0    # Maksimum değer
                    )
