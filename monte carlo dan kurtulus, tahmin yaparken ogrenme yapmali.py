...
class BitcoinPredictorApp(QMainWindow):
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        """
        Derin öğrenme ve monte carlo simülasyonunu birleştiren tahmin fonksiyonu
        """
        try:
            ...
            # Son sequence ve başlangıç değerleri
            last_sequence = self.X_scaled[-60:].copy()
            current_price = self.df['Close'].iloc[-1].item()  # .item() kullanarak tekil değeri al
            all_time_high = max(self.df['Close'].max().item(), current_price)  # .item() kullanarak tekil değeri al
            ...
            for i in range(days_to_predict):
                ...
                for sim in range(n_simulations):
                    try:
                        ...
                        rf_input = last_sequence[-1:].reshape(1, -1)
                        rf_pred = self.ensemble_model.rf_model.predict(rf_input)[0]
                        
                        # XGBoost tahmini
                        xgb_pred = self.ensemble_model.xgb_model.predict(rf_input)[0]
                        
                        # LightGBM tahmini
                        lgbm_pred = self.ensemble_model.lgbm_model.predict(rf_input)[0]
                        ...
                        # Market döngüsü etkisi
                        market_cycle = np.sin(2 * np.pi * i / 365) * 0.1
                        halving_cycle = np.sin(2 * np.pi * i / (4 * 365)) * 0.2
                        
                        # Fiyat sınırlamaları
                        if weighted_pred > all_time_high * 1.5:
                            weighted_pred = all_time_high * (1 + np.random.uniform(0.2, 0.5))
                        elif weighted_pred < all_time_high * 0.2:
                            weighted_pred = all_time_high * (0.2 + np.random.uniform(0, 0.1))
                        
                        simulation_predictions.append(weighted_pred)
                        
                    except Exception as sim_error:
                        print(f"Simülasyon hatası: {str(sim_error)}")
                        continue
                
                if not simulation_predictions:
                    raise ValueError("Geçerli simülasyon sonucu yok")
                    
                # Simülasyon sonuçlarını analiz et
                pred_mean = np.mean(simulation_predictions)
                pred_std = np.std(simulation_predictions)
                confidence = 1.0 / (1.0 + pred_std / pred_mean)
                
                # Trend belirle
                if len(predictions) > 0:
                    price_change = (pred_mean - predictions[-1]) / predictions[-1]
                ...
                # Sonuçları kaydet
                predictions.append(float(pred_mean))
                ...
                try:
                    features_dict = self._calculate_advanced_features(
                        ...
                        pred_mean,
                        trend_momentum=(pred_mean - current_price) / current_price
                    )
                ...
                # All-time high ve current price güncelle
                all_time_high = max(all_time_high, pred_mean)
                current_price = pred_mean
                
                # İlerleme göster
                if i % 10 == 0:
                    print(f"Tahmin ilerleme: {i+1}/{days_to_predict} gün")
