...
logger = logging.getLogger('ModelEvaluator2')
...
class ModelEvaluator2:
    ...
    def evaluate_specific_prediction(self, prediction_date, target_date):
        ...
        """
        try:
            self.logger.info(f"Değerlendirme başlatılıyor - Tahmin tarihi: {prediction_date}, Hedef tarih: {target_date}")
            ...
            # DataFrame index'inin timezone'ını al
            df_tz = self.model.df.index.tz
            
            # Timezone'ları eşitle
            if df_tz is not None:
                if prediction_date.tz is None:
                    prediction_date = prediction_date.tz_localize(df_tz)
            ...
            self.logger.debug(f"Tarihler dönüştürüldü ve timezone'lar eşitlendi")
            
            # Veri aralığı kontrolü
            min_date = self.model.df.index.min()
            max_date = self.model.df.index.max()
            
            self.logger.info(f"Veri aralığı kontrolü yapılıyor. Mevcut aralık: {min_date.strftime('%d.%m.%Y')} - {max_date.strftime('%d.%m.%Y')}")
            
            if prediction_date < min_date or prediction_date > max_date:
                error_msg = (f"Tahmin tarihi ({prediction_date.strftime('%d.%m.%Y')}) veri aralığı dışında. " 
                        f"Mevcut veri aralığı: {min_date.strftime('%d.%m.%Y')} - {max_date.strftime('%d.%m.%Y')}")
                self.logger.error(error_msg)
                raise ValueError(error_msg)
                
            if target_date < min_date or target_date > max_date:
                error_msg = (f"Hedef tarih ({target_date.strftime('%d.%m.%Y')}) veri aralığı dışında. "
                        f"Mevcut veri aralığı: {min_date.strftime('%d.%m.%Y')} - {max_date.strftime('%d.%m.%Y')}")
                self.logger.error(error_msg)
                raise ValueError(error_msg)
            ...
            self.logger.debug(f"Tarihsel veri alındı: {len(historical_data)} kayıt")
            
            # Gerçek değeri al
            target_mask = (self.model.df.index.normalize() == target_date.normalize())
            target_data = self.model.df[target_mask]
            
            if target_data.empty:
                self.logger.warning("Tam tarih eşleşmesi bulunamadı, en yakın tarih aranıyor")
                nearest_idx = self.model.df.index.get_indexer([target_date], method='nearest')[0]
                if nearest_idx < 0:
                    raise ValueError("Belirtilen tarih için yakın bir eşleşme bulunamadı")
                actual_price = float(self.model.df.iloc[nearest_idx]['Close'])
                target_date = self.model.df.index[nearest_idx]
            else:
                actual_price = float(target_data['Close'].iloc[0])
                
            self.logger.info(f"Gerçek değer: ${actual_price:,.2f}")
            
            # Tahmin yap
            self.logger.info("Tahmin yapılıyor...")
            self.model.df = historical_data
            predictions = self.model.predict_next_days(target_date.strftime('%Y-%m-%d'))
            
            if predictions.empty:
                raise ValueError("Tahmin oluşturulamadı")
            
            # Hedef tarihe en yakın tahmini bul
            closest_pred_idx = None
            min_diff = float('inf')
            target_norm = target_date.normalize()
            
            for idx, row in predictions.iterrows():
                pred_date = pd.to_datetime(row['Tarih']).normalize()
                if pred_date.tz is None and target_norm.tz is not None:
                    pred_date = pred_date.tz_localize(target_norm.tz)
                elif pred_date.tz is not None and target_norm.tz is None:
                    pred_date = pred_date.tz_localize(None)
                    
                diff = abs(pred_date.value - target_norm.value)  # nanoseconds cinsinden fark
                if diff < min_diff:
                    min_diff = diff
                    closest_pred_idx = idx
            
            if closest_pred_idx is None:
                raise ValueError("Hedef tarih için uygun tahmin bulunamadı")
                
            target_prediction = float(predictions.iloc[closest_pred_idx]['Tahmin'])
            self.logger.info(f"Tahmin edilen değer: ${target_prediction:,.2f}")
            self.logger.info(f"Tahmin tarihi: {predictions.iloc[closest_pred_idx]['Tarih']}")
            
            # Modeli eski haline getir
            self.model.prepare_data_and_models()
            self.logger.debug("Model orijinal durumuna döndürüldü")
            
            # Hata hesapla
            error = abs(actual_price - target_prediction)
            error_percentage = (error / actual_price) * 100
            
            self.logger.info(f"Mutlak hata: ${error:,.2f}")
            self.logger.info(f"Hata oranı: %{error_percentage:.2f}")
            ...
            self.logger.info("Değerlendirme başarıyla tamamlandı")
            return results
            
        except Exception as e:
            error_msg = f"Değerlendirme hatası: {str(e)}"
