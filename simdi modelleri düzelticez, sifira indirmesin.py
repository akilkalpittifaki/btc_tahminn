...
# make_prediction fonksiyonunda tarih kontrolünü düzeltmek gerekiyor:

    def make_prediction(self):
        try:
            # Model kontrolü
            if not hasattr(self, 'lstm_model') or self.lstm_model is None:  # Değiştirildi
                QMessageBox.warning(self, "Uyarı", "Model henüz eğitilmedi, lütfen bekleyin!")
                return
            
            # Tarih kontrolünü kaldır çünkü geçmiş tarihleri de tahmin edebilmeliyiz
            ...
            # Tahminleri çiz - hata kontrolü ekle
            try:
                predictions_df.set_index('Tarih')['Tahmin'].plot(
                    ax=self.ax,
                    label='Tahminler',
                    linestyle='--',
                    linewidth=2,
                ...
                print(f"Grafik çizim hatası: {plot_error}")
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        ...
        try:
            ...
            current_price = float(self.df['Close'].iloc[-1])
            historical_min = float(self.df['Close'].min())  # Tarihi minimum değer
            historical_max = float(self.df['Close'].max())  # Tarihi maksimum değer
            
            # Trend fazları ve özellikleri - güncellendi
            trend_phases = {
                'bull': {
                    'duration': (60, 120),  # Daha uzun bull run
                    'strength': (0.2, 0.5),  # Daha makul artış
                    'volatility': (0.02, 0.03)
                },
                'bear': {
                    'duration': (30, 60),
                    'strength': (-0.2, -0.05),  # Düşüşler sınırlandı
            ...
            for i in range(days_to_predict):
                try:
                    ...
                    # Final tahmin hesaplama - güncellendi
                    pred_price = base_pred * (1 + trend_strength)
                    pred_price *= (1 + halving_effect * 0.3)  # Halving etkisi artırıldı
                    pred_price *= (1 + market_cycle_effect * 0.2)  # Market cycle etkisi azaltıldı
                    pred_price *= (1 + momentum_effect * 0.1)  # Momentum etkisi azaltıldı
                    
                    # Halving sonrası boost etkisi
                    if current_date > last_halving:
                        days_since_halving = (current_date - last_halving).days
                        if days_since_halving < 365:  # Halving sonrası ilk yıl
                            halving_boost = 1 + (0.2 * (1 - days_since_halving/365))  # Maksimum %20 artış
                            pred_price *= halving_boost
                    
                    # Makul tahmin aralığı kontrolü - güncellendi
                    time_factor = min(1.0, i/(days_to_predict * 0.5))  # Zamanla artan genişlik
                    min_allowed_price = max(
                        historical_min * 0.8,  # Tarihi minimumun %80'i
                        current_price * (1 - 0.3 * time_factor)  # Maksimum %30 düşüş
                    )
                    max_allowed_price = min(
                        historical_max * 2.5,  # Tarihi maksimumun 2.5 katı
                        current_price * (1 + 0.5 * time_factor)  # Maksimum %50 artış
                    )
                    
                    # Tahmin sınırlarını uygula
                    pred_price = np.clip(pred_price, min_allowed_price, max_allowed_price)
                    
                    # NaN kontrolü
                    if np.isnan(pred_price):
                        pred_price = current_price
                    
                    # Tahmin güvenilirliği hesaplama - güncellendi
                    confidence = 1.0 - (i / days_to_predict) * 0.4  # Zaman ilerledikçe güven daha yavaş azalır
                    confidence *= 1.0 - abs(trend_strength) * 0.5  # Trend etkisi azaltıldı
                    confidence *= 1.0 - trend_volatility * 0.5  # Volatilite etkisi azaltıldı
