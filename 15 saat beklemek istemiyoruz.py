...
class BitcoinPredictorApp(QMainWindow):
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        """
        Daha gerçekçi tahmin fonksiyonu - kademeli ve zikzaklı tahminler üretir
        """
        try:
            predictions = []
            dates = []
            ...
            # Başlangıç değerlerini ayarla - Series'den güvenli şekilde float al
            try:
                current_price = float(self.df['Close'].iloc[-1])
                historical_high = float(self.df['Close'].max())
                historical_volatility = float(self.df['Close'].pct_change().std())
            except Exception as e:
                print(f"Veri dönüşüm hatası: {str(e)}")
                raise
            ...
            cycle_phases = {
                'bull': {
                    ...
                    'total_growth': (1.2, 1.5),  # %20-50 büyüme
                    'volatility': (0.02, 0.04)
                },
                'bear': {
                    'duration': (15, 30),   # 15-30 gün
                    'max_decline': 0.3,     # Maksimum %30 düşüş
                    'min_decline': 0.05,    # Minimum %5 düşüş
                    'volatility': (0.02, 0.04)
                },
                'accumulation': {
                    'duration': (20, 40),   # 20-40 gün
                    'range': 0.1,           # %10 fiyat aralığı
            ...
            for i in range(days_to_predict):
                ...
                volatility = np.random.uniform(*cycle_phases[current_phase]['volatility'])
                if current_phase == 'bull' or current_phase == 'bear':
                    # Trend yönünde hareket
                    remaining_days = max(1, phase_duration - days_in_phase)
                    target_return = (phase_target / current_price) ** (1 / remaining_days)
                    expected_change = target_return - 1
                else:
                    # Yatay hareket
                    remaining_days = max(1, phase_duration - days_in_phase)
                    expected_change = (phase_target - current_price) / (remaining_days * current_price)
                
                # Rastgele değişim ekle
                daily_volatility = volatility * np.random.normal(0, 1)
                price_change = expected_change + daily_volatility
                
                # Yeni fiyat
                current_price *= (1 + price_change)
                
                # Fiyat sınırları
                current_price = max(current_price, all_time_high * 0.5)  # En düşük zirvenin %50'si
                current_price = min(current_price, all_time_high * 2.0)  # En fazla zirvenin 2 katı
                ...
                time_factor = 1.0 - (i / days_to_predict) * 0.5  # Zaman ilerledikçe güven azalır
                trend_factor = 0.8 if current_phase == 'accumulation' else 0.6  # Akümülasyon fazında daha yüksek güven
                confidence = max(0.3, time_factor * trend_factor)
