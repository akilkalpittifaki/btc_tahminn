...
class BitcoinPredictorApp(QMainWindow):
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        """
        Daha gerçekçi tahmin fonksiyonu - makul düzeltmeler ve zirve yenilemeleriyle
        """
        try:
            predictions = []
            dates = []
            ...
            cycle_phases = {
                'bull': {
                    ...
                    'total_growth': (2.0, 3.0),  # %200-300 büyüme
                    'volatility': (0.02, 0.04)
                },
                'bear': {
                    'duration': (60, 90),   # 2-3 ay
                    'max_decline': 0.8,     # Maksimum %80 düşüş
                    'min_decline': 0.2,     # Minimum %20 düşüş
                    'volatility': (0.03, 0.05)
                },
                'accumulation': {
                    'duration': (45, 90),   # 1.5-3 ay
                    'range': 0.2,           # %20 fiyat aralığı
            ...
            peaks_count = 0  # Zirve deneme sayısı
            failed_breakout_count = 0  # Başarısız zirve kırma denemesi
            ...
            for i in range(days_to_predict):
                ...
                if days_in_phase >= phase_duration:
                    ...
                    if current_phase == 'accumulation':
                        ...
                        # Zirve kırma mantığı
                        if failed_breakout_count >= 2:  # İki başarısız denemeden sonra
                            phase_target = max(phase_target, all_time_high * 1.2)  # Kesin zirve kırma
                            failed_breakout_count = 0
                        elif current_price > all_time_high * 0.7:  # Zirveye yakınsa
                            phase_target = max(phase_target, all_time_high * np.random.uniform(0.9, 1.3))
                            
                    elif current_phase == 'bull':
                        current_phase = 'bear'
                        # Düşüş miktarı - zirvenin en fazla %80'i
                ...
                if current_phase == 'bull' or current_phase == 'bear':
                    ...
                    target_return = (phase_target / current_price) ** (1 / (phase_duration - days_in_phase))
                    expected_change = target_return - 1
                else:
                    # Yatay hareket
                    expected_change = (phase_target - current_price) / (phase_duration - days_in_phase)
