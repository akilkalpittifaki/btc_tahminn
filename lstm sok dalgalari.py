...
class BitcoinPredictorApp(QMainWindow):
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        ...
        try:
            ...
            current_trend = (last_price - price_30d_ago) / price_30d_ago  # Son 30 günlük trend
            trend_momentum = current_trend * 2  # Trend momentumunu artır
            ...
            for i in range(days_to_predict):
                ...
                # Geliştirilmiş rastgelelik ve trend
                # 1. Temel volatilite (kısa vadeli)
                short_term_volatility = np.std(self.df['Returns'].iloc[-30:]) * 400  # Artırıldı
                
                # 2. Uzun vadeli trend etkisi
                trend_effect = 1 + (trend_momentum * (i/days_to_predict))  # Zamanla artan trend etkisi
                
                # 3. Periyodik dalgalanmalar
                cycle_effect = np.sin(i / 10) * 0.05  # 10 günlük periyodik dalgalanma
                
                # 4. Rastgele şoklar (düşük olasılıklı büyük hareketler)
                shock = 0
                if np.random.random() < 0.1:  # %10 olasılıkla
                    shock = np.random.normal(0, short_term_volatility * 2)
                ...
                features_dict = {
                    'Open': pred_price,
                    'High': pred_price * (1 + np.random.uniform(0, 0.03)),  # Artırıldı
                    'Low': pred_price * (1 - np.random.uniform(0, 0.03)),   # Artırıldı
                    'Close': pred_price,
                    'Volume': float(self.df['Volume'].mean() * np.random.uniform(0.5, 2.0)),  # Daha geniş aralık
                    'Returns': float((pred_price - predictions[-1])/predictions[-1] if len(predictions) > 0 else 0),
                    'Volatility': float(np.std(predictions[-7:]) if len(predictions) > 1 else short_term_volatility),
                    ...
                    'RSI': float(50 + np.random.uniform(-20, 20)),  # Daha geniş RSI aralığı
                    'MACD': float(trend_momentum * np.random.uniform(-1, 1)),
                    'Signal_Line': float(trend_momentum * np.random.uniform(-0.5, 0.5)),
                    'BB_middle': float(pred_price),
                    'BB_upper': float(pred_price * (1 + 0.02 * (1 + abs(trend_momentum)))),
                    'BB_lower': float(pred_price * (1 - 0.02 * (1 + abs(trend_momentum))))
                }
                ...
                # Trend momentumunu güncelle
                if len(predictions) > 1:
                    short_trend = (predictions[-1] - predictions[-2]) / predictions[-2]
                    trend_momentum = trend_momentum * 0.95 + short_trend * 0.05  # Trend momentumu güncelle
