...
class BitcoinPredictorApp(QMainWindow):
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        """
        Daha gerçekçi tahmin fonksiyonu - düşük volatilite ve makul fiyat hareketleri
        """
        try:
            ...
            # Daha gerçekçi parametreler
            max_daily_change = min(0.05, historical_volatility)  # Maksimum günlük değişimi sınırla
            trend_strength = 0.002  # Trend gücünü azalt
            volatility_factor = 0.3  # Volatiliteyi azalt
            
            # Trend fazları
            trend_phases = {
                'bull': {
                    'duration': (30, 60),  # Daha uzun süreler
                    'strength': (0.001, 0.003),  # Daha düşük artışlar
                    'volatility': (0.001, 0.002)
                },
                'bear': {
                    'duration': (20, 40),
                    'strength': (-0.002, -0.001),
                    'volatility': (0.001, 0.002)
                },
                'accumulation': {
                    'duration': (15, 30),
                    'strength': (-0.0005, 0.0005),
                    'volatility': (0.0005, 0.001)
                }
            }
                ...
                try:
                    ...
                    if trend_days_left <= 0:
                        ...
                        if current_trend == 'accumulation':
                            current_trend = 'bull' if np.random.random() > 0.6 else 'accumulation'
                        elif current_trend == 'bull':
                            current_trend = 'bear' if np.random.random() > 0.7 else 'bull'
                        else:  # bear
                            current_trend = 'accumulation'
                    ...
                    trend_momentum = trend_momentum * 0.95 + trend_strength * 0.05
                    
                    # Volatilite hesaplama
                    daily_volatility = np.random.uniform(
                        *trend_phases[current_trend]['volatility']
                    ) * volatility_factor
                    
                    # Fiyat değişimi hesaplama
                    price_change = (
                        trend_momentum +  # Trend etkisi
                        np.random.normal(0, daily_volatility)  # Rastgele değişim
                    )
                    ...
                    # Yeni fiyatı hesapla
                    pred_price = current_price * (1 + price_change)
