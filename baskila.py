...
class BitcoinPredictorApp(QMainWindow):
    ...
    def setup_model_weights_ui(self):
        ...
        self.weight_sliders = {}
        # LSTM kaldırıldı, diğer modeller için ağırlıklar
        for model in ['Linear', 'XGBoost', 'Prophet', 'SARIMA']:
            slider = QSlider(Qt.Horizontal)
            slider.setMinimum(0)
            slider.setMaximum(100)
            slider.setValue(25)
            
            label = QLabel(f"{model}: 25%")
            # Model ismini lambda içinde sabit tut
        ...
        self.main_layout.addWidget(weights_widget)       
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        ...
        try:
            ...
            linear_preds = self.get_linear_predictions(days_to_predict)
            ...
            for i in range(days_to_predict):
                ...
                weights = {
                    'Linear': self.weight_sliders['Linear'].value() / 100,
                ...
                std_dev = np.std([linear_preds[i], xgb_preds[i], 
                                prophet_preds[i], sarima_preds[i]])
    ...
    def prepare_training_data(self):
        ...
        self.y = self.df['Close'].values.ravel()  # .values.ravel() kullanılarak tek boyutlu hale getirildi
    ...
    def get_linear_predictions(self, days_to_predict):
        """Linear Regression tahminleri"""
        predictions = []
        current_data = self.df.iloc[-1:].copy()
        
        for _ in range(days_to_predict):
            features = self.linear_scaler.transform(current_data[self.linear_features])
            pred = float(self.linear_model.predict(features.reshape(1, -1))[0])  # Veri boyutunu uygun hale getirmek için .reshape(1, -1) kullanıldı
    ...
    def get_xgb_predictions(self, days_to_predict):
        ...
        for _ in range(days_to_predict):
            ...
            pred = float(self.xgb_model.predict(features.reshape(1, -1))[0])  # Veri boyutunu uygun hale getirmek için .reshape(1, -1) kullanıldı
            predictions.append(pred)
            self.update_technical_features(current_data, pred, predictions)
