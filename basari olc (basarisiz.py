...
class PredictionMetrics:
    def calculate_metrics(self, actual, predicted):
        ...
        try:
            # Dizilerin aynı uzunlukta olmasını sağla
...
class EnhancedMarketPatterns:
    def __init__(self):
        self.patterns = {
            'bull_run': {
                'duration': (30, 90),
                'daily_change': (0.005, 0.03),
                'volatility': (0.02, 0.04),
                'momentum_factor': 1.2
            },
            'correction': {
                'duration': (14, 45),
                'daily_change': (-0.02, -0.005),
                'volatility': (0.03, 0.05),
                'momentum_factor': 0.8
            },
            'accumulation': {
                'duration': (20, 60),
                'daily_change': (-0.01, 0.01),
                'volatility': (0.01, 0.02),
                'momentum_factor': 1.0
            },
            'distribution': {
                'duration': (20, 40),
                'daily_change': (-0.015, 0.015),
                'volatility': (0.02, 0.03),
                'momentum_factor': 0.9
            }
        }
        
    def generate_pattern(self, pattern_type, base_price, duration):
        """Gerçekçi fiyat pattern'i oluştur"""
        pattern = self.patterns[pattern_type]
        prices = [base_price]
        
        momentum = 1.0
        for _ in range(duration):
            daily_change = np.random.uniform(*pattern['daily_change'])
            volatility = np.random.uniform(*pattern['volatility'])
            
            # Momentum etkisi
            momentum *= pattern['momentum_factor']
            momentum = np.clip(momentum, 0.5, 1.5)
            
            # Fiyat değişimi
            price_change = daily_change * momentum
...
class BitcoinPredictorApp(QMainWindow):
    ...
    def _calculate_advanced_features(self, predictions, current_price, trend_momentum=0):
        ...
        try:
            ...
            # Volume hesaplama - FutureWarning düzeltmesi
            mean_volume = float(self.df['Volume'].iloc[:].mean()) if isinstance(self.df['Volume'], pd.Series) else float(self.df['Volume'])
            ...
            features = {
                ...
                'Momentum_Short': float(self.momentum_short.iloc[0] if isinstance(self.momentum_short, pd.Series) else 
                                    (self.momentum_short[0] if isinstance(self.momentum_short, np.ndarray) else self.momentum_short)),
                'Momentum_Long': float(self.momentum_long.iloc[0] if isinstance(self.momentum_long, pd.Series) else 
                                (self.momentum_long[0] if isinstance(self.momentum_long, np.ndarray) else self.momentum_long)),
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        """
        Geliştirilmiş tahmin fonksiyonu - daha gerçekçi fiyat hareketleri için düzeltildi
        """
        try:
            ...
            trend_phases = {
                ...
                'accumulation': {
                    'duration': (7, 21),
                    'strength': (-0.008, 0.008),
                    'volatility': (0.01, 0.02)
                },
                'distribution': {
                    'duration': (7, 21),
                    'strength': (-0.01, 0.01),
                    'volatility': (0.015, 0.025)
                }
            }
            
            # Başlangıç trendi ve momentum
            current_trend = np.random.choice(['bull', 'bear', 'accumulation', 'distribution'])
            trend_days_left = np.random.randint(*trend_phases[current_trend]['duration'])
            trend_momentum = 0
            
            # Halving cycle parametreleri
            last_halving = datetime(2024, 4, 1)
            halving_cycle_days = 4 * 365
            market_cycle_days = 365
            
            # LSTM tahminlerinin güvenilirliği için pencere
            lstm_confidence_window = min(30, days_to_predict)
            
            for i in range(days_to_predict):
                try:
                    current_date = last_real_date + timedelta(days=i+1)
                    
                    # Halving ve market cycle etkileri
                    days_since_halving = (current_date - last_halving).days
                    halving_effect = 0.05 * np.sin(2 * np.pi * days_since_halving / halving_cycle_days)
                    market_cycle_effect = 0.03 * np.sin(2 * np.pi * days_since_halving / market_cycle_days)
                    
                    # Trend değişimi kontrolü
                    if trend_days_left <= 0:
                        prev_trend = current_trend
                        market_sentiment = np.random.random()
                        
                        if current_trend == 'accumulation':
                            current_trend = 'bull' if market_sentiment > 0.4 else 'distribution'
                        elif current_trend == 'bull':
                            current_trend = 'distribution' if market_sentiment > 0.7 else 'bull'
                        elif current_trend == 'distribution':
                            current_trend = 'bear' if market_sentiment > 0.6 else 'accumulation'
                        else:  # bear
                            current_trend = 'accumulation' if market_sentiment > 0.7 else 'bear'
                    ...
                    # Trend ve volatilite hesaplama
                    base_strength = np.random.uniform(*trend_phases[current_trend]['strength'])
                    current_volatility = np.random.uniform(*trend_phases[current_trend]['volatility'])
                    
                    # Trend momentumu güncelleme
                    trend_momentum = trend_momentum * 0.95 + base_strength * 0.05
                    
                    # LSTM tahminini al
                    try:
                        lstm_pred = self.ensemble_model.predict(last_sequence)
                        if isinstance(lstm_pred, (list, np.ndarray)):
                            lstm_pred = float(lstm_pred[0])
                    except Exception:
                        lstm_pred = current_price
                    
                    # LSTM ağırlığını hesapla
                    lstm_weight = max(0.1, min(0.7, 1.0 - (i / lstm_confidence_window)))
                    market_weight = 1.0 - lstm_weight
                    
                    # Fiyat değişimi hesaplama
                    lstm_change = (lstm_pred - current_price) / current_price
                    market_change = (
                        trend_momentum +
                        halving_effect +
                        market_cycle_effect +
                        np.random.normal(0, current_volatility)
                    )
                    
                    # Ağırlıklı fiyat değişimi
                    total_change = lstm_change * lstm_weight + market_change * market_weight
                    
                    # Değişimi sınırla
                    total_change = np.clip(total_change, -0.1, 0.1)  # Maksimum %10 günlük değişim
                    
                    # Yeni fiyat hesapla
                    new_price = current_price * (1 + total_change)
                    
                    # Volatilite bazlı noise ekle
                    noise = np.random.normal(0, historical_volatility * current_price * 0.05)
                    new_price += noise
                    
                    # Global sınırlar içinde tut
                    new_price = np.clip(new_price, historical_min * 0.7, historical_max * 1.5)
                    ...
                    # Güven skoru hesapla
                    time_factor = 1.0 - (i / days_to_predict) * 0.5
                    volatility_factor = 1.0 - min(1.0, current_volatility / 0.04)
                    trend_continuity = 1.0 - (trend_days_left / trend_phases[current_trend]['duration'][1])
                    
                    confidence = np.clip(
                        (time_factor + volatility_factor + trend_continuity) / 3,
                        0.3, 0.9
                    )
                    ...
                    logging.error(f"İterasyon hatası (gün {i}): {str(iteration_error)}")
                    continue
            
            # Aşırı gürültüyü filtrele
            predictions = pd.Series(predictions).rolling(window=3, min_periods=1, center=True).mean().values
            ...
            logging.error(f"Tahmin işlemi hatası: {str(e)}")
            return pd.DataFrame()
