...
class BitcoinPredictorApp(QMainWindow):
    ...
    def prepare_training_data(self):
        ...
        # Linear Regression ve XGBoost için özellikler
        ...
        self.y_scaled = self.lstm_scaler.fit_transform(self.y.values.reshape(-1, 1))

        # LSTM için veri hazırlığı
        self.prepare_lstm_training_data()
    ...
    def train_models(self):
        ...
        self.lstm_model.add(LSTM(units=50, return_sequences=True, input_shape=(self.X_lstm.shape[1], 1)))
        self.lstm_model.add(LSTM(units=50, return_sequences=False))
        self.lstm_model.add(Dense(units=1))

        self.lstm_model.compile(optimizer='adam', loss='mean_squared_error')
        self.lstm_model.fit(self.X_lstm, self.y_lstm, epochs=20, batch_size=32)
    ...
    def make_prediction(self):
        ...
        try:
            ...
            # Kullanılan model bilgilerini ekrana yazdır
            self.results_text.append("Kullanılan Model Bilgileri:\n")
            for weight in self.model_weights:
                model_info = (
                    f"Tarih: {weight['Date'].strftime('%Y-%m-%d')}\n"
                    f"Linear Regression Ağırlığı: %{weight['Linear']*100:.0f} - Tahmin: ${weight['Linear_Pred']:.2f}\n"
                    f"XGBoost Ağırlığı: %{weight['XGBoost']*100:.0f} - Tahmin: ${weight['XGBoost_Pred']:.2f}\n"
                    f"LSTM Ağırlığı: %{weight['LSTM']*100:.0f} - Tahmin: ${weight['LSTM_Pred']:.2f}\n"
                    f"{'='*50}\n"
                )
                self.results_text.append(model_info)
    ...
    def predict_future_date(self, days_to_predict, last_real_date):
        ...
        try:
            ...
            for i in range(days_to_predict):
                ...
                if i <= 7:
                    linear_weight = 0.05  # Linear ağırlık
                    xgb_weight = 0.05
                    lstm_weight = 0.9
                else:
                    linear_weight = 0.05  # XGBoost ve LSTM ağırlıklı
                    xgb_weight = 0.05
                    lstm_weight = 0.9
                ...
                # LSTM tahmini
                lstm_input = self.y_scaled[-60:].reshape(1, 60, 1)
                lstm_pred_scaled = self.lstm_model.predict(lstm_input)[0][0]
                lstm_pred = self.lstm_scaler.inverse_transform([[lstm_pred_scaled]])[0][0]
                
                # Ağırlıklı tahmin
                next_price = (linear_pred * linear_weight) + (xgb_pred * xgb_weight) + (lstm_pred * lstm_weight)
